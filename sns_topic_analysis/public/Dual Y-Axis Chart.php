...
<body>
    <div class="left">
        ...
        <p><b>시각화 목적:</b><br>
        시간대별 주제 집중도 및 전체 발화량을 시각화하여, 
        사용자의 <b>활동 패턴과 주제별 집중도</b>를 비교·분석하는 근거 자료로 활용 가능</p>
    </div>

    <div class="right">
        <div class="chart-container">
            <h1>시간대별 토픽 추세 (Dual Y-Axis)</h1>
            <canvas id="topicChart"></canvas>
        </div>
        <div class="chart-container">
            <h1>시간대별 전체 발화량 (Line Chart)</h1>
    ...
    <script>
        ...
        // 1. 시간대별 토픽 추세 그래프 (Dual Y Axis)
        new Chart(ctx, {
            type: 'line',
            data: {
                labels: timeRanges,
                datasets: chartData.map((d, i) => {
                    const useRightAxis = d.label === "개인 및 관계";
                    return {
                        label: d.label,
                        data: d.data,
                        borderColor: useRightAxis ? "#003f5c" : colors[i % colors.length],
                        backgroundColor: useRightAxis ? "#003f5c" : colors[i % colors.length],
                        borderWidth: useRightAxis ? 4 : 2,
                        pointRadius: useRightAxis ? 6 : 3,
                        tension: 0.25,
            ...
            options: {
                ...
                scales: {
                    yLeft: {
                        ...
                        min: 0,
                        max: 700,
                        title: { display: true, text: "그 외 토픽", color: '#666' }
                    },
                    yRight: {
                        type: 'linear',
                        position: 'right',
                        ...
                        title: { display: true, text: "개인 및 관계", color: '#003f5c', font: { weight: 'bold' } },
                        ticks: { color: '#003f5c', font: { weight: 'bold' } }
                    }
                }
            }
        });

        // 2. 시간대별 전체 발화량 추세 그래프
        new Chart(ctx2, {
            type: 'line',
            data: {
                labels: timeRanges,
