...
W_IMG = 0.6
W_AUD = 0.4
MAX_PAIRS_PER_CLASS = 120  # 클래스당 최대 쌍 수 증가
...
# =========================================================
# 🚀 평가 (logits-level weighted fusion)
# =========================================================
y_true, y_pred = [], []
with torch.no_grad():
    for img, aud, y in tqdm(loader, desc="Evaluating late fusion (logits)"):
        img, aud = img.to(DEVICE), aud.to(DEVICE)
        logits_img = img_model(img)
        logits_aud = aud_model(aud)

        # ✅ logits 단계에서 가중 평균 → softmax
...
print(f"✅ Late Fusion v2 Accuracy: {acc*100:.2f}% | Macro-F1: {macro_f1*100:.2f}%")
