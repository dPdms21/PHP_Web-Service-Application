# py_light/audio/train_audio_mobilenet_v3.py
...
OUTPUT_DIR = "outputs_light/audio_mobilenet_v3"
...
# =========================================================
# 🧠 모델 정의 (MobileNetV3-Small)
# =========================================================
weights = models.MobileNet_V3_Small_Weights.DEFAULT
model = models.mobilenet_v3_small(weights=weights)

in_features = model.classifier[3].in_features
model.classifier[3] = nn.Linear(in_features, len(HAR_LABELS))
...
for epoch in range(1, EPOCHS + 1):
    model.train()
    pbar = tqdm(train_loader, desc=f"[MobileNetV3] Epoch {epoch}/{EPOCHS}")
    for x, y in pbar:
        x, y = x.to(DEVICE), y.to(DEVICE)
        optimizer.zero_grad()
        out = model(x)
